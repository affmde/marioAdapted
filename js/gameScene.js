...
class GameScene extends Phaser.Scene{
    ...
    create(){
        ...
        arrows = this.add.image(100, 500, 'ArrowsController').setAlpha(0.7).setScrollFactor(0).setInteractive();
        controlA = this.add.image(650,500, 'ControlA').setAlpha(0.5).setScale(0.6).setScrollFactor(0).setInteractive();
        squareLeft = this.add.image(50, 500, 'squareFake').setScale(0.7).setInteractive().setScrollFactor(0).setAlpha(0.1);
        squareRight = this.add.image(150, 500, 'squareFake').setScale(0.7).setInteractive().setScrollFactor(0).setAlpha(0.1);
        squareUp = this.add.image(100, 450, 'squareFake').setScale(0.7).setInteractive().setScrollFactor(0).setAlpha(0.1);
        squareDown = this.add.image(100, 550, 'squareFake').setScale(0.7).setInteractive().setScrollFactor(0).setAlpha(0.1);
        //Create Live Hearts
    ...
    update(){
        ...
        squareLeft.on('pointerdown', ()=>{
            this.player.setVelocityX(-160)
            squareLeft.setAlpha(0.3)
        })
        squareLeft.on('pointerup', ()=>{
            this.player.setVelocityX(0);
            squareLeft.setAlpha(0.1)
        })
        squareRight.on('pointerdown', ()=>{
            this.player.setVelocityX(160)
            squareRight.setAlpha(0.3)
        })
        squareRight.on('pointerup', ()=>{
            this.player.setVelocityX(0)
            squareRight.setAlpha(0.1)
        })
        controlA.on('pointerdown', ()=>{
            if(this.player.body.touching.down)
            this.player.setVelocityY(-380)
            controlA.setScale(0.5)
        })
